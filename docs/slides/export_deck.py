...
def main():
    ...
    with pymupdf.open(args.pdf) as document:
        ...
        presentation.core_properties.title = (
            "Echocardiography Preprocessing: Data, Methods and Results" if is_english
            else "心脏超声数据预处理：阶段汇报"
        )
        presentation.core_properties.subject = (
            "Full-slide-image presentation from LaTeX Beamer PDF" if is_english
            else "LaTeX Beamer PDF 的整页图像放映版"
        )
        presentation.core_properties.author = "eco_prepro"
        editing_note = (
            "Full-slide-image presentation; edit the LaTeX source to change content."
            if is_english else "整页图像放映版；修改内容请编辑 LaTeX 源码。"
        )
